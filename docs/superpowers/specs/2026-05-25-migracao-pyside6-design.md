...
- Novo `.spec` PyInstaller (`ProcessadorOcorrencias-v2.0.spec`); ajustes pontuais em `deploy.py`.
...
- [auto_update.py](../../../auto_update.py) — totalmente intacto (filename vem do servidor).
...
```
┌─ 1 · PDF de jornada ───────────────────────────────┐
│  [DropZone: arraste o PDF ou clique pra selecionar]│
...
Dropzones aceitam tanto clique (abre `QFileDialog`) quanto arrastar (eventos `dragEnter` / `drop`). O botão "Processar" habilita só quando há exatamente 1 PDF e 1 XLSX (estado READY). `ProcessadorOcorrencias.processar()` aceita um único `pdf_path` por execução — não é processamento em lote.
...
Mesma arquitetura visual (wizard + LogPanel + worker em `QThread`), porém com os inputs específicos de `ProcessadorVTCaixa.processar(fonte_path, xls_path, output_path, usar_ia, api_key, model_id)`:

- Card 1: **Fonte Nautilus** — DropZone que aceita PDF **ou** XLSX (estende a regra de aceite da `DropZone` por extensão).
- Card 2: **Excel cadastral** — DropZone XLS/XLSX.
- Card 3: **Opções** — checkbox "Usar IA (Gemini)" + dropdown de modelo (default `gemini-2.5-flash`). Quando IA marcada, valida que há API key em config.
- Saída: CSV (não XLSX como Ocorrências) — o seletor de saída é um `QFileDialog.getSaveFileName` com filtro `.csv`.
...
**Nenhuma mudança no código** — o nome do exe a ser baixado vem do `filename` no manifest do servidor ([auto_update.py:48-49](../../../auto_update.py#L48)), não está hardcoded no cliente. Basta que o admin publique a v2.0 no servidor de licença e o auto-update aplicará.
...
11. Gera `ProcessadorOcorrencias-v2.0.spec`, ajusta `deploy.py`; bump `APP_VERSION = "2.0"`.
